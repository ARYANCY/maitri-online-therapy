...
export default function ReminderBell({ onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!sendAt) {
      setInfo(
        t(
          ...
          "Please choose a valid date/time for custom option."
        )
      );
      return;
    }
    ...
    setInfo("");
    try {
      // full try/catch for detailed error
      const res = await API.post("/api/reminders", { message, sendAt });
      setInfo(
        t("reminder.scheduled", "Reminder scheduled!") +
          " ✅"
      );
            ...
            <ul className="reminder-list">
              {reminders.length === 0 && (
                <li className="reminder-empty">
                  {t("reminder.empty", "No reminders yet.")}
                </li>
              )}
              {reminders.map((r) => (
                <li className="reminder-item" key={r._id}>
                  <span>
                    {r.message} —{" "}
                    <small>{new Date(r.sendAt).toLocaleString()}</small>
                  </span>
                  <button
